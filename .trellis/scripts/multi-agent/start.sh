...
claude -p --agent dispatch --session-id "${SESSION_ID}" --dangerously-skip-permissions --output-format stream-json --verbose "Execute the task pipeline strictly from task.json phases (implement -> check -> finish -> create-pr). Do not invoke any Skill or slash-command workflow. Run dispatch actions directly."
RUNNER_EOF
