...
func (h *Handler) RegisterRoutes(v1 fiber.Router, authService *auth.Service) {
	challengeGroup := v1.Group("/challenges", middleware.Auth(authService))

	challengeGroup.Get("", h.List)
	challengeGroup.Get("/:id", h.Get)

	challengeGroup.Post("", middleware.RequireRoles(auth.RoleAdmin), h.Create)
	challengeGroup.Put("/:id", middleware.RequireRoles(auth.RoleAdmin), h.Update)
	challengeGroup.Delete("/:id", middleware.RequireRoles(auth.RoleAdmin), h.Delete)
}

func (h *Handler) Create(c *fiber.Ctx) error {
	var req CreateChallengeRequest
	if err := c.BodyParser(&req); err != nil {
		return apperrors.BadRequest("CHALLENGE_INVALID_PAYLOAD", "Invalid request payload")
	}
...
func (h *Handler) List(c *fiber.Ctx) error {
	...
	if err != nil {
		return apperrors.BadRequest("CHALLENGE_INVALID_QUERY", "limit must be an integer")
	}
	offset, err := parseQueryInt(c.Query("offset"), 0)
	if err != nil {
		return apperrors.BadRequest("CHALLENGE_INVALID_QUERY", "offset must be an integer")
	}
